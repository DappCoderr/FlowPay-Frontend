...
import { useCreateAndScheduleStream } from '@/hooks';
...
export function CreateSubscriptionModal({ open, onClose, onCreate }) {
  ...
  // Use the hook for creating and scheduling streams
  const { mutate: createAndScheduleStream, isPending: isMutating } =
    useCreateAndScheduleStream();
  ...
  const isValid = () => {
    ...
    if (
      recipients.some((r) => r.shareType === 'percentage') &&
      totalPercentage !== 100
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Derive delaySeconds from selected date/time relative to now
    const now = new Date();
    const target = new Date(`${scheduledDate}T${scheduledTime}:00Z`);
    const delaySeconds = Math.max(0, (target.getTime() - now.getTime()) / 1000);

    // Map recurrence to recurrenceRaw (UInt8) and totalRounds
    let recurrenceRaw = 0; // 0 = once
    let totalRounds = 1;
    ...
    setTxError(null);
    createAndScheduleStream(
      {
        recipients: recipientsWithAmounts.map((r) => r.address),
        amounts: recipientsWithAmounts.map((r) =>
          r.amount.toFixed(2).toString()
        ),
        ...
        onStatus: (status) => {
          if (status === 'sealed') {
            // Success - notify parent with optimistic data
            const optimistic = {
              id: `pending-${Date.now()}`,
              label: name || `Subscription (pending)`,
              totalPerPayout: recipientsWithAmounts.reduce(
                (s, r) => s + r.amount,
                0
              ),
              recipients: recipientsWithAmounts,
              completedPayouts: 0,
              totalPayouts: totalRounds,
              status: 'pending',
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {isMutating && (
          <div className="p-3 mb-2 rounded bg-white/5 border border-white/10 text-sm">
            <strong>Status:</strong> Creating stream...
            {txError && <div className="text-red-400 mt-1">{txError}</div>}
          </div>
        )}
        {txError && !isMutating && (
          <div className="p-3 mb-2 rounded bg-red-500/10 border border-red-500/20 text-sm">
            <strong className="text-red-400">Error:</strong>
            <div className="text-red-400 mt-1">{txError}</div>
          </div>
        )}
        <Input
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button
            ...
            disabled={!!isMutating}
          >
            {isMutating ? 'Creating...' : 'Create subscription'}
          </Button>
        </div>
      </form>
    </Modal>
