...
import { sendFlowTokens } from '@/flow/flowpay'
...
export function SendFlowModal({ open, onClose, userAddress, balance }) {
  ...
  const isValid = () => {
    ...
    if (balance && Number(amount) > Number(balance)) e.amount = 'Insufficient balance'
    setErrors(e)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await sendFlowTokens(recipient.trim(), amount)
      setTxError(null)
      onClose()
      resetForm()
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-white/60">Balance: {balance ? `${balance} FLOW` : '—'}</span>
          </div>
          <div className="p-3 rounded-lg border border-white/20 bg-white/5">
            <p className="text-sm font-mono break-all text-white/70">{userAddress}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <label className="text-sm font-medium text-white/80">Amount (FLOW)</label>
            <button
              type="button"
